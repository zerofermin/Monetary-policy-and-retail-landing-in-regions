...
def detect_outlier_regions_iqr(df, key_vars, min_obs=60, max_na_pct=0.3):
    ...
    for region in df['Region'].unique():
        ...
        cols = [c for c in key_vars if c in region_df.columns]
        reg_data = region_df[cols].dropna()
        
        # Подсчёт пропусков по индикаторам
        missing_pct_by_col = {}
        for col in cols:
            try:
                missing_pct_by_col[col] = region_df[col].isna().mean()
            except Exception:
                missing_pct_by_col[col] = np.nan
        
        # 1. Минимум наблюдений
        n_obs = len(reg_data)
        total_rows = len(region_df)
        na_pct = 1 - (n_obs / total_rows) if total_rows > 0 else 1
        
        missing_cols = [col for col in cols
                        if col in missing_pct_by_col
                        and pd.notna(missing_pct_by_col[col])
                        and missing_pct_by_col[col] > max_na_pct]
        if not missing_cols:
            missing_cols = [col for col in cols
                            if col in missing_pct_by_col
                            and pd.notna(missing_pct_by_col[col])
                            and missing_pct_by_col[col] > 0]
            missing_cols = sorted(missing_cols,
                                  key=lambda c: missing_pct_by_col[c],
                                  reverse=True)[:5]
        bad_indicators_missing = ", ".join(missing_cols)
        missing_details = "; ".join([f"{col} ({missing_pct_by_col[col]:.1%})" for col in missing_cols])
        
        if n_obs < min_obs:
            indicator_problem = f'Мало данных (<{min_obs})'
            if missing_details:
                indicator_problem += f'; Пропуски: {missing_details}'
            region_stats.append({
                'Region': region,
                'N_obs': n_obs,
                'NA_pct': f'{na_pct:.1%}',
                'Bad_Indicators': bad_indicators_missing,
                'Indicator_Problem': indicator_problem,
        ...
        if na_pct > max_na_pct:
            indicator_problem = f'Много пропусков (>{max_na_pct*100}%)'
            if missing_details:
                indicator_problem += f'; Пропуски: {missing_details}'
            region_stats.append({
                'Region': region,
        ...
        for col in key_vars:
            if col in reg_data.columns:
                ...
                iqr_outliers_by_col[col] = outliers
        ...
        if iqr_pct > 0.2:  # >20% выбросов по IQR
            ...
            iqr_bad_cols = [col for col in cols if iqr_outliers_by_col.get(col, 0) > 0]
            bad_indicators = ", ".join(iqr_bad_cols)
            if iqr_bad_cols:
                iqr_details = []
                for col in iqr_bad_cols:
                    pct = iqr_outliers_by_col[col] / n_obs if n_obs > 0 else 0
                    iqr_details.append(f"{col}={iqr_outliers_by_col[col]} ({pct:.1%})")
                indicator_problem = "IQR выбросы: " + "; ".join(iqr_details)
            status = 'EXCLUDE'
    ...
    result = pd.DataFrame(region_stats)
    col_order = [
        'Region',
        'N_obs',
        'NA_pct',
    ...
    col_order = [c for c in col_order if c in result.columns] + [c for c in result.columns if c not in col_order]
...
def run_panel_regressions(df_input, df_reg, dependent_var, exog_vars_initial, cov_type, cluster_entity=None, df_name=None, intercept=True, time_fe=False):
    from linearmodels.panel import PanelOLS, RandomEffects, PooledOLS

    cov_key = str(cov_type).strip().lower() if cov_type is not None else ""
    if cov_key == "clustered":
        cov_type_use = "clustered"
        cov_kwargs = {"cluster_entity": True}
    elif cov_key in ("driscoll-kraay", "dk"):
        cov_type_use = "driscoll-kraay"
        cov_kwargs = {}
    elif cov_key == "robust":
        cov_type_use = "robust"
        cov_kwargs = {}
    elif cov_key == "two-way":
        cov_type_use = "clustered"
        cov_kwargs = {}
    else:
        raise ValueError("cov_type must be one of: 'clustered', 'driscoll-kraay', 'dk', 'robust', 'two-way'")
    ...
    if cov_key == "two-way":
        clusters = pd.DataFrame(
            {
                "Region": df_clean.index.get_level_values("Region"),
                "Date": df_clean.index.get_level_values("Date"),
            },
            index=df_clean.index,
        )
        cov_kwargs = {"clusters": clusters}
    if cov_key == "two-way":
        clusters = pd.DataFrame(
            {
                "Region": df_clean.index.get_level_values("Region"),
    ...
    if time_fe:
        time_dummies = pd.get_dummies(df_clean.index.get_level_values("Date"))
        time_dummies.index = df_clean.index
    ...
    try:
        ...
        pooled_res = pooled_mod.fit(cov_type=cov_type_use, **cov_kwargs)
    ...
    try:
        ...
        re_res = re_mod.fit(cov_type=cov_type_use, **cov_kwargs)
...
def run_spec_tests_robust(y, X, pooled_res, fe_res, re_res, pooled_success, fe_success, re_success):
    ...
    print("\n" + "="*70)
    print("ТЕСТЫ СПЕЦИФИКАЦИИ (ROBUST)")
    print("="*70)

    # ТЕСТ ХАУСМАНА
    print("\n ТЕСТ ХАУСМАНА (FE vs RE, robust)")
    print("-"*70)
    ...
    if fe_success and re_success:
        try:
            ...
            fe_idx = getattr(fe_params, "index", None)
            re_idx = getattr(re_params, "index", None)
            if fe_idx is None or re_idx is None:
                print("Предупреждение: индексы параметров недоступны, тест пропущен.")
            else:
                common = fe_idx.intersection(re_idx)
                dropped = [name for name in re_idx if name not in fe_idx]
                if dropped:
                    print(f"Предупреждение: FE отбросила переменные: {', '.join(dropped)}")
                if len(common) == 0:
                    print("Предупреждение: нет общих параметров для сравнения, тест пропущен.")
                else:
                    b_diff = fe_params.loc[common] - re_params.loc[common]
                    ...
                    try:
                        ...
                        print("Предупреждение: V_diff сингулярна, Hausman пропущен.")
                        inv_v_diff = None

                    if inv_v_diff is not None:
                        H = b_diff.values @ inv_v_diff @ b_diff.values
                        p_val = 1 - chi2.cdf(H, df=len(common))

                        print(f"H-статистика: {H:.4f}")
                        print(f"p-значение: {p_val:.6f}")
                        print(f"df: {len(common)}")
    ...
    # ТЕСТ БРЕУША-ПАГАНА
    print("\n ТЕСТ БРЕУША-ПАГАНА (RE vs Pooled, classical)")
    print("-"*70)
    ...
    # F-ТЕСТ
    print("\n F-ТЕСТ (FE vs Pooled, robust)")
    print("-"*70)

    if fe_success and pooled_success:
        try:
            f_test = getattr(fe_res, "f_pooled", None)
            if f_test is None:
                print("Предупреждение: fe_res.f_pooled недоступен, тест пропущен.")
            else:
                f_obj = f_test() if callable(f_test) else f_test
                stat = getattr(f_obj, "stat", None)
                p_val = getattr(f_obj, "pval", None)
                df = getattr(f_obj, "df", None)
                if stat is None or p_val is None:
                    print("Предупреждение: нет статистики/ p-значения, тест пропущен.")
                else:
                    print(f"F-статистика: {stat:.4f}")
                    print(f"p-значение: {p_val:.6f}")
                    if df is not None:
                        print(f"df: {df}")
...
def run_panel_model_diagnostics(
    ...
    def _pesaran_cd(resids):
        ...
        if n_pairs == 0:
            return None
        # Use integer overlap counts, not boolean logic.
        valid = (~wide.isna()).astype(int)
...
def collect_spec_test_pvalues(y, X, pooled_res, fe_res, re_res, pooled_success, fe_success, re_success):
    from scipy.stats import chi2, f
    ...
    return {
        "Hausman (FE vs RE) p-value": hausman_pval,
        "Breusch-Pagan LM (RE vs Pooled) p-value": bp_lm_pval,
        "F-test (FE vs Pooled) p-value": f_pval
    }


def collect_diagnostics_pvalues(
    y,
    X,
    pooled_res=None,
    fe_res=None,
    ...
    def _modified_wald_pvalue(resids):
        if not isinstance(resids.index, pd.MultiIndex) or len(resids.index.names) < 2:
            return None
        idx_names = list(resids.index.names)
        if "Region" in idx_names and "Date" in idx_names:
    ...
    def _normality_pvalues(resids):
        if resids is None or len(resids) < 3:
            return None, None
        jb_stat, jb_p, _, _ = jarque_bera(resids)
        if len(resids) > max_shapiro_n:
            shapiro_sample = resids.sample(max_shapiro_n, random_state=42)
        else:
            shapiro_sample = resids
        shapiro_stat, shapiro_p = stats.shapiro(shapiro_sample)
        return float(jb_p), float(shapiro_p)

    def _wooldridge_pvalue(resids):
        if not isinstance(resids.index, pd.MultiIndex) or len(resids.index.names) < 2:
            return None
        idx_names = list(resids.index.names)
        if "Region" in idx_names and "Date" in idx_names:
    ...
    def _shock_wald_pvalue(res, shock_vars_in):
        if res is None:
            return None
        shock_vars_in = shock_vars_in or []
        params = getattr(res, "params", None)
    ...
    def _flatten_shock_vars(values):
        if values is None:
            return []
        flat = []
        for item in values:
    ...
    shock_vars = _flatten_shock_vars(shock_vars or ["Covid_dum", "Sank_dum"])

    y_aligned, X_aligned, _ = _align(y, X, None)
    dwh_pval = _dwh_min_pvalue(y_aligned, X_aligned)
    ...
    def _fill_for_model(label, res, success, is_fe=False):
        if not success or res is None:
            return
        resids = _get_residuals(res)
        if resids is None:
            return
        _, X_m, resids = _align(y, X, resids)
        if X_m is None or resids is None:
            return
        resids = resids.dropna()
        if resids.empty:
        ...
        if is_fe:
            out[label]["Modified Wald p-value"] = _modified_wald_pvalue(resids)
        else:
            exog = _get_model_exog(res, resids.index)
            if exog is not None:
                try:
            ...
            if exog is not None:
                ...
                if not exog.empty:
                    ...
                    if hetero_add_constant and not _has_constant(exog):
                        exog = sm.add_constant(exog, has_constant="add")
                    if _hetero_feasible(exog):
                        try:
                            _, bp_p, _, _ = het_breuschpagan(resids_het, exog)
                            out[label]["Breusch-Pagan p-value"] = float(bp_p)
                        except Exception:
                            out[label]["Breusch-Pagan p-value"] = None
                        try:
                            _, w_p, _, _ = het_white(resids_het, exog)
                            out[label]["White p-value"] = float(w_p)
                        except Exception:
                            out[label]["White p-value"] = None

        jb_p, shapiro_p = _normality_pvalues(resids)
        out[label]["Jarque-Bera p-value"] = jb_p
        out[label]["Shapiro-Wilk p-value"] = shapiro_p

        try:
            _ = durbin_watson(resids.values)
            out[label]["Durbin-Watson p-value"] = None
        except Exception:
            out[label]["Durbin-Watson p-value"] = None

        out[label]["Wooldridge AR(1) p-value"] = _wooldridge_pvalue(resids)
        out[label]["Pesaran CD p-value"] = _pesaran_cd_pvalue(resids)
        out[label]["Shock Wald p-value"] = _shock_wald_pvalue(res, shock_vars)
        out[label]["DWH min p-value"] = dwh_pval

    _fill_for_model("POOL", pooled_res, pooled_success, is_fe=False)
    _fill_for_model("FE", fe_res, fe_success, is_fe=True)
    _fill_for_model("RE", re_res, re_success, is_fe=False)

    return out


def collect_all_test_pvalues(
    y,
    X,
    pooled_res,
    fe_res,
    ...
    spec_tests = collect_spec_test_pvalues(
        y, X, pooled_res, fe_res, re_res, pooled_success, fe_success, re_success
    )
    diag_tests = collect_diagnostics_pvalues(
        y,
        X,
        ...
        hetero_add_constant=hetero_add_constant,
    )
    return {
