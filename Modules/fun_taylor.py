...
def extract_ardl_coefficients(result, y_name, print_results=True):
    ...
    if 'Inflation_gap' in params.index:
        coeffs['alpha'] = params['Inflation_gap']
        coeffs['alpha_pval'] = pvalues['Inflation_gap']
    
    # Извлечение мю (уверенность)
    if 'Ent_conf_ind_total' in params.index:
        coeffs['mu'] = params['Ent_conf_ind_total']
        coeffs['mu_pval'] = pvalues['Ent_conf_ind_total']
